...
def generate_with_retry(model, text, audio_prompt_path, exaggeration, temperature, cfg_weight, max_retries=3):
    ...
    print(f"🚀 Long text ({text_length} chars) → GPU with retry: '{text[:30]}{'...' if len(text) > 30 else ''}'")
    ...
    for retry in range(max_retries):
            ...
            if retry > 0:
                print(f"✅ GPU generation successful on retry {retry}/{max_retries}!")
            else:
            ...
            if ("srcIndex < srcSelectDimSize" in str(e) or 
                ...
                if retry < max_retries - 1:
                    ...
                    print(f"❌ GPU failed after {max_retries} retries, falling back to CPU...")
